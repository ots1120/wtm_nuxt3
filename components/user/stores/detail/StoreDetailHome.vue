<template>
  <div class="mt-3 space-y-4">
    ...
    <!-- SNS 정보가 있을 경우에만 표시 -->
    <div v-if="storeSns?.url" class="flex items-center gap-4">
      <span class="font-semibold text-gray-700">SNS</span>
      <a :href="storeSns.url" target="_blank" class="text-blue-500">{{
        storeSns.url
      }}</a>
    </div>
    ...
    <div class="flex items-center gap-4">
      ...
      <span>{{ Ticket.price ? `${Ticket.price}원` : '가격 정보 없음' }}</span>
    </div>
  </div>
</template>
