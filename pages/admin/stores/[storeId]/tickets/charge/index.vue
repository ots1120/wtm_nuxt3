  ...
  <div class="container mx-auto p-4">
    ...
    <div class="bg-white shadow-md rounded-lg p-4 max-w-md mx-auto">
      ...
      >
        ...
        <p class="text-green-600">식권 충전이 승인되었습니다.</p>
      </div>
      <div
...
const handleQRCodeDecode = async (decodedText: string) => {
  console.log('QR 코드 스캔 성공 (Recharge):', decodedText);
  ...
  try {
    ...
    console.error('QR 코드 데이터 파싱 실패 (Recharge):', error);
  ...
  if (decodedData.value.storeId !== storeId) {
    console.error(
      `Store ID 불일치 (Recharge): QR 코드의 Store ID (${decodedData.value.storeId})와 현재 Store ID (${storeId})가 다릅니다.`,
    );
    approvalStatus.value = 'rejected';
  ...
  // QR 데이터 type 검증
  if (decodedData.value.type !== 'purchase') {
    console.error(
      `QR 타입 불일치 (Recharge): 충전 메뉴에서 사용하려면 QR 타입이 'purchase'여야 합니다.`,
    );
    approvalStatus.value = 'rejected';
    errorMessage.value = '충전 메뉴에서 사용 가능한 QR 코드가 아닙니다.';
    return;
  }

  // 선택된 식권 ID 확인
  if (!selectedTicketId.value) {
    console.error('식권이 선택되지 않았습니다. (Recharge)');
    approvalStatus.value = 'rejected';
  ...
  // API 요청 보내기
  try {
    const response = await fetch(`${baseUrl}/api/v1/admin/ticket/purchase`, {
      method: 'POST',
    ...
    console.log('Recharge 요청 승인됨');
  } catch (error) {
    approvalStatus.value = 'rejected';
    errorMessage.value = '식권 충전 요청이 실패했습니다.';
    console.error('Recharge 요청 실패:', error);
  } finally {
